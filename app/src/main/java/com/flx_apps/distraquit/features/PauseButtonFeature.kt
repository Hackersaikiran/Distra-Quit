...
import com.flx_apps.distraquit.features.PauseButtonFeature.hardwareKey
import com.flx_apps.distraquit.system_integration.DistraQuitAccessibilityService
import com.flx_apps.distraquit.system_integration.PauseInteractionService
import com.flx_apps.distraquit.system_integration.PauseTileService
import com.flx_apps.distraquit.ui.screens.feature.pause_button.PauseButtonFeatureSettingsSection
import java.util.concurrent.TimeUnit

/**
 * The [PauseButtonFeature] can be used to pause DistraQuit for a defined amount of time. It will
 * call [Feature.onPause] for all active features and DistraQuit will ignore all accessibility
 * events until the pause is over.
 *
 * The user can pause DistraQuit:
 *
 * - by long-pressing the hardware key defined in [hardwareKey] for 2 seconds (see
 * [DistraQuitAccessibilityService.onKeyEvent],
 *
 * - by clicking the pause button in the quick settings tile (see [PauseTileService]), or
 *
 * - by setting DistraQuit as default assistant app (see [PauseInteractionService]).
 */
object PauseButtonFeature : Feature() {
    ...
    /**
     * The hardware key that can be used to pause DistraQuit by long-pressing it for 2 seconds.
     */
    ...
    override fun onPause(context: Context) {
        ...
        DistraQuitAccessibilityService.instance?.updateForegroundNotification()
    }
    ...
    fun pauseFeatures(context: Context, stop: Boolean = false) {
        ...
        DistraQuitAccessibilityService.updateState()
    }

    fun resume() {
        pauseUntil = 0
        DistraQuitAccessibilityService.instance.takeIf { it != null }?.let { service ->
