...
import com.flx_apps.distraquit.OneMinuteInMs
...
object FeaturesProvider {
    ...
    var activeFeatures = mutableSetOf<Feature>()
        get() {
            ...
            if (System.currentTimeMillis() - lastActiveFeaturesReload > OneMinuteInMs) {
                ...
                if (DistraQuitAccessibilityService.updateState() != DistraQuitState.Inactive) {
                    field.forEach { feature ->
                        if (!newActiveFeatures.contains(feature)) {
                            feature.onPause(DistraQuitApplication.appContext)
                        }
                    }
                }
    ...
    fun startOrStopFeature(feature: Feature) {
        ...
        if (DistraQuitAccessibilityService.updateState() == DistraQuitState.Active) {
            // if DistraQuit is running, call onStart() or onPause() for the feature
            if (feature.isActive()) feature.onStart(DistraQuitApplication.appContext) else feature.onPause(
                DistraQuitApplication.appContext
            )
        }
    }
