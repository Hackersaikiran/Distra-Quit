package com.flx_apps.distraquit.features
...
import com.flx_apps.distraquit.data.DataStoreProperty
import com.flx_apps.distraquit.data.DataStorePropertyTransformer
import com.flx_apps.distraquit.feature_types.AppExceptionListType
import com.flx_apps.distraquit.feature_types.Feature
import com.flx_apps.distraquit.feature_types.FeatureTexts
import com.flx_apps.distraquit.feature_types.NeedsDrawOverlayPermissionFeature
import com.flx_apps.distraquit.feature_types.NeedsPermissionsFeature
...
import com.flx_apps.distraquit.feature_types.ScreenTimeTrackingFeature
import com.flx_apps.distraquit.feature_types.SupportsAppExceptionsFeature
...
import com.flx_apps.distraquit.system_integration.OverlayService
import com.flx_apps.distraquit.ui.screens.feature.disable_apps.AppDisabledOverlayService
import com.flx_apps.distraquit.ui.screens.feature.disable_apps.DisableAppsFeatureSettingsSection
...
/**
 * This feature can disable apps. If DistraQuit has DEVICE_ADMIN permission, it can completely
...
object DisableAppsFeature : Feature(), OnAppOpenedSubscriptionFeature,
    ...
    SupportsAppExceptionsFeature by SupportsAppExceptionsFeature.Impl(DisableAppsFeatureId, AppExceptionListType.ONLY_LIST),
     ...
     * disabled, while the feature and DistraQuit are active.
     */
    ...
    override fun onPause(context: Context) {
        if (operationMode == DisableAppsMode.DEACTIVATE) {
            // if the apps are deactivated, we need to reactivate them when DistraQuit is paused
    ...
    override fun onAppOpened(
        ...
        // close any previous tracking session before handling the current app
        ...
        if (allowedDailyScreenTime > 0L && getCurrentUsedUpScreenTime() < allowedDailyScreenTime) {
    ...
    fun setAppsDeactivated(
        ...
        if (skipOperation || !DistraQuitDeviceAdminReceiver.isGranted(context)) {
        ...
        disableableApps.forEach {
            val appDeactivated = devicePolicyManager.setApplicationHidden(
                ComponentName(context, DistraQuitDeviceAdminReceiver::class.java), it, deactivated
            )
