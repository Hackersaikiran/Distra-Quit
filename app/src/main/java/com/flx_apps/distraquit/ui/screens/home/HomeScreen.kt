...
import com.flx_apps.distraquit.BuildConfig
import com.flx_apps.distraquit.R
import com.flx_apps.distraquit.feature_types.Feature
...
import com.flx_apps.distraquit.system_integration.UsageStatsProvider
...
import com.flx_apps.distraquit.ui.widgets.InfoCard
import com.flx_apps.distraquit.ui.widgets.SimpleListTile
import com.flx_apps.distraquit.util.NavigationUtil
import com.flx_apps.distraquit.util.observeAsState
import com.flx_apps.distraquit.util.toHrMinString
 ...
 * start/stop DistraQuit and displays some stats about the screen time of the current day.
 */
@Composable
fun HomeScreen(
    homeViewModel: HomeViewModel = viewModel()
) {
    val distraQuitState: DistraQuitState = homeViewModel.distraQuitState.collectAsState().value
    val snackbarHostState = remember { SnackbarHostState() }
    Scaffold(snackbarHost = {
        SnackbarHost(hostState = snackbarHostState)
    }, topBar = {
        AppBar(distraQuitState)
    }, floatingActionButton = {
        StartStopActionButton(distraQuitState)
    }) {
...
/**
 * The top app bar of the home screen. It displays the current state of DistraQuit.
 * @see DistraQuitState
 */
@Composable
@OptIn(ExperimentalMaterial3Api::class)
private fun AppBar(
    distraQuitState: DistraQuitState, navViewModel: NavViewModel = NavViewModel.navViewModel()
) {
    TopAppBar(
        title = {
            Column {
                ...
                Row(verticalAlignment = Alignment.CenterVertically) {
                    StatusIndicator(
                        indicatorColor = colorResource(
                            id = when (distraQuitState) {
                                DistraQuitState.Active -> R.color.green
                                DistraQuitState.Inactive -> R.color.gray
                                DistraQuitState.Paused -> R.color.yellow
                            }
                        )
                    )
                    Text(
                        text = stringResource(
                            id = when (distraQuitState) {
                                DistraQuitState.Active -> R.string.home_state_active
                                DistraQuitState.Inactive -> R.string.home_state_inactive
                                DistraQuitState.Paused -> R.string.home_state_paused
                            }, if (distraQuitState == DistraQuitState.Paused) {
...
/**
 * A [ExtendedFloatingActionButton] that starts or stops DistraQuit when clicked.
 */
@Composable
private fun StartStopActionButton(
    distraQuitState: DistraQuitState, homeViewModel: HomeViewModel = viewModel()
) {
    ExtendedFloatingActionButton(
        text = { Text(text = stringResource(id = if (distraQuitState != DistraQuitState.Inactive) R.string.home_stop else R.string.home_start)) },
        icon = {
            Icon(
                painter = if (distraQuitState != DistraQuitState.Inactive) painterResource(
                    id = R.drawable.ic_stop
                ) else painterResource(
                    id = R.drawable.ic_start
                ), contentDescription = "Run/Stop DistraQuit"
            )
        },
        onClick = {
            homeViewModel.toggleDistraQuitIsRunning()
        })
}
...
/**
 * A tile that uninstalls DistraQuit when clicked. It is only shown if the device admin permission
 * is granted. A confirmation dialog is shown when before revoking the device admin permission.
 */
@Composable
fun UninstallDistraQuitTile(viewModel: HomeViewModel = viewModel()) {
    // don't show the uninstall tile if the device admin permission is not granted
    if (!DistraQuitDeviceAdminReceiver.isGranted(LocalContext.current)) return
...
fun ScreenTimeChart(navViewModel: NavViewModel = NavViewModel.navViewModel()) {
    ...
    val context = LocalContext.current
    val usageByApp = UsageStatsProvider.getUpdatedAppUsageTodayByEvents()
    val selectedSlice = remember { mutableStateOf<PieChartData.Slice?>(null) }


    // only show the top 5 apps in the chart
    val chartStats = usageByApp.entries.sortedByDescending { it.value }.take(5)
    val screenTime = usageByApp.values.sum()
    ...
    val otherTime = (screenTime - chartStats.sumOf { it.value }).coerceAtLeast(1L).toFloat()
    val donutChartData = PieChartData(
        slices = chartStats.mapIndexed { index, entry ->
            val label = try {
                packageManager.getApplicationInfo(entry.key, 0)
                ...
                if (usageByApp.isNotEmpty()) {
                    if (selectedSlice.value != null) {
                        ...
                        if (selectedIndex >= 0 && selectedIndex < chartStats.count()) {
                            ...
                            Text(
                                text = selectedStat.value.milliseconds.toHrMinString(),
                                style = MaterialTheme.typography.titleLarge
                            )
                            Text(
                                text = try {
                                    packageManager.getApplicationInfo(selectedStat.key, 0)
                                        .loadLabel(packageManager)
                                        .toString()
                                } catch (_: Exception) {
                                    selectedStat.key
